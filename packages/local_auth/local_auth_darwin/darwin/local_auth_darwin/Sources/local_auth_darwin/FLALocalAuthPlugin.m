...
- (void)clearBiometricCheckingWithError:
    (FlutterError *_Nullable __autoreleasing *_Nonnull)error {
    [[NSUserDefaults standardUserDefaults] removeObjectForKey: BIOMETRIC_CHECK_KEY];
}
...
static NSString* const BIOMETRIC_CHECK_KEY = @"check-biometrics";
...
- (NSString*)getBiometricState {
    ...
    if (domainState != nil) {
        return [self base64UrlSafe: domainState];
    }

    return nil;
}

- (BOOL)compareBiometricState {
    NSString* current = [self getBiometricState];
    
    if (current == nil) {
        return @NO;
    }
    
    NSString* previous = [[NSUserDefaults standardUserDefaults] stringForKey:BIOMETRIC_CHECK_KEY];
    
    if (previous == nil) {
        [[NSUserDefaults standardUserDefaults] setObject: current forKey:BIOMETRIC_CHECK_KEY];
        return @YES;
    }
...
- (void)handleAuthReplyWithSuccess:(BOOL)success
  ...
  if (success) {
      if ([options checkBiometricInvalidationForKey]) {
          [self handleSucceeded:YES
                 withBiometrics:[self compareBiometricState]
                 withCompletion:completion];
      } else {
...
- (void)handleSucceeded:(BOOL)succeeded
         withCompletion:(nonnull FLADAuthCompletion)completion {
...
- (void)handleSucceeded:(BOOL)succeeded
         withBiometrics:(BOOL)validated
         withCompletion:(nonnull FLADAuthCompletion)completion {
  completion([FLADAuthResultDetails
              makeWithResult:(succeeded ? (validated ? FLADAuthResultSuccessValidated : FLADAuthResultSuccessInvalidated)
                              : FLADAuthResultFailure)
                   errorMessage:nil
                   errorDetails:nil],
             nil);
}
